...
class Meal:
  ...
  def __init__(self, data):
      ...
      self.nutrition = self.__parse_nutrition(self.data["nutrition"]["nutrients"])
      self.ingredients = self.__parse_ingredients(self.data["nutrition"]["ingredients"])
  ...
  def __parse_nutrition(self, nutrients):
      temp = {}
      for i in range(len(nutrients)):
        temp[nutrients[i]["name"].lower()] = nutrients[i]["percentOfDailyNeeds"]
      return [temp.get(Meal.nutrients_list[i], 0) for i in range(len(Meal.nutrients_list))]
...
class MealPlan:
  ...
  def __init__(self, meals, mealTypes, restrictions):
    if "vegan" in restrictions:
      meals = [meal for meal in meals if meal["vegan"]]
    elif "vegetarian" in restrictions:
      meals = [meal for meal in meals if meal["vegetarian"]]
    if "glutenFree" in restrictions:
      meals = [meal for meal in meals if meal["glutenFree"]]
    if "dairyFree" in restrictions:
      meals = [meal for meal in meals if meal["dairyFree"]]
    meals = [meal for meal in meals if "breakfast" in meal["dishTypes"] and mealTypes[0] or "lunch" in meal["dishTypes"] and mealTypes[1] or "dinner" in meal["dishTypes"] and mealTypes[2]]
  ...
  def __preference_cost(self, preferences):
    aggregate = {}
    for meal in self.meals:
  ...
  def __ingredient_cost(self, ingredients):
    aggregate = {}
    for meal in self.meals:
    ...
    for id_ in aggregate:
      cost += (aggregate[id_] - ingredients.get(id_, 0)) / aggregate[id_]
    return cost

  def __nutrition_cost(self, dv):
    return np.sqrt(np.sum(MealPlan.NUTRITION_WEIGHTS * (sum([meal.nutrition for meal in self.meals]) - dv) ** 2))
  
  def total_cost(self, preferences, ingredients, dv):
    return np.dot(np.array([self.__variety_cost(), self.__time_cost(), self.__preference_cost(preferences), self.__ingredient_cost(ingredients), self.__nutrition_cost(dv)]), np.array([1, 1, 1, 1, 1]))
...
class User:
  ...
  def __init__(self, demographics, goals, restrictions, preferences, meals, restaurants, ingredients):
